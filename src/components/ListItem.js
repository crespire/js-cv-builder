  ...
  render() {
    ...
    return (
      <div className="container grid grid-cols-10 grid-rows-4 gap-4 border-solid border-black border p-6">
        ...
        <button className="print:hidden bg-slate-200 col-start-10 row-start-1 col-span-1 row-span-2" onClick={() => { toggleEdit(itemID)} }>Edit</button>
        <button className="print:hidden bg-slate-200 col-start-10 row-start-3 col-span-1 row-span-2" onClick={() => { deleteItem(itemID)} }>Delete</button>
      </div>
    );
  }
}
